...
stock_hist['9-day'] = stock_hist['Close'].rolling(9).mean()
stock_hist['21-day'] = stock_hist['Close'].rolling(21).mean()
...
with plt.style.context('ggplot'):
    ...
    plt.plot(stock_hist['9-day'][-120:])
    plt.plot(stock_hist['21-day'][-120:])
    plt.legend(loc=2)
    
    # this does not take in account equality
stock_hist['position'] = np.where(stock_hist['9-day'] > stock_hist['21-day'], 1, 0)
stock_hist['position'] = np.where(stock_hist['9-day'] < stock_hist['21-day'] , -1, stock_hist['position'])
